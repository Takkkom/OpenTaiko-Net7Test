	...
	internal class CAct演奏AVI : CActivity
	{
            ...
            if ( ( nチャンネル番号 == 0x54 || nチャンネル番号 == 0x5A ) && TJAPlayer3.ConfigIni.bAVI有効 )
            {
                this.rAVI = rAVI;
                {
                    ...
                    if ( fAVIアスペクト比 < 1.77f )
                    {
                        ...
                        this.vclip = new Vector3D<float>(1.42f, 1.42f, 1f);
                        #endregion
                    }
                    if (this.tx描画用 == null)
                    {
                        this.tx描画用 = new CTexture((int)this.framewidth, (int)this.frameheight);
                    }
                    ...
                    this.smallvc = new Vector3D<float>(f拡大率x, f拡大率y, 1f);
                    #endregion
                }
		...
		public unsafe int t進行描画( int x, int y )
		{
			if ( !base.b活性化してない )
			{
				...
				Size szフレーム幅 = new Size(1280, 720);
                ...
                if( ( this.tx描画用 != null ))
                {
                    if ( ( this.bDShowクリップを再生している == true ) )
                    {
                        #region[ ワイドクリップ ]
                        #endregion
                    }
        ...
        public void tReset()
        {
            if( this.tx描画用 != null && this.tx窓描画用 != null )
            {
				...
				this.tx描画用 = new CTexture( 1280, 720 );
				this.tx窓描画用 = new CTexture( 1280, 720 );
            }
        ...
        public Vector3D<float> smallvc;
        private Vector3D<float> vclip;
